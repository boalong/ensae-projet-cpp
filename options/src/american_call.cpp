...
american_call::american_call(double spotprice, double strikeprice, double maturity, double volatility, double r) : option(spotprice, strikeprice, maturity, volatility, r) {}

american_call::~american_call() {}


double american_call::price_BSM()
{
...
double american_call::price_BT(int N)
{
...
double american_call::price_MC(int N_timesteps, int N_simulations)
{
    double sum = 0;
    double S;
    double diff;
...
void american_call::replication()
{
    double d1 = (log(get_spotprice() / get_strikeprice()) + (get_r() + 0.5 * get_volatility() * get_volatility()) * get_maturity()) / (get_volatility() * sqrt(get_maturity()));
    double d2 = d1 - get_volatility() * sqrt(get_maturity());
    ...
    cout<<"To replicate this American call, buy "<<N_shares<<" in shares of the stock \nand borrow "<<N_bonds<<" in bonds at the risk-free interest rate."<<endl;
}
